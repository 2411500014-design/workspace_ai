...
- **2026-09-25**: **Aplikasi disempurnakan agar stabil dan mudah dicoba**, diuji tanpa AI.
  - **Coba dengan contoh project** di layar sambutan: satu ketukan membuat project skripsi contoh lengkap (dua dokumen, brief, rencana yang diterima, satu task selesai, satu task berjalan, dan satu usulan dari catatan bimbingan) dalam bahasa yang dipilih.
  - **Buka di HP**: kartu di Pengaturan menampilkan alamat laptop untuk dibuka di browser ponsel pada Wi-Fi yang sama, atau langkah yang belum dilakukan (server belum dibuka untuk jaringan, web belum di-build).
  - Bug yang diperbaiki, antara lain: task yang sedang dikerjakan tidak muncul di Hari Ini kalau jadwalnya nanti; tombol Batalkan dan aksi lain bisa error setelah kartunya hilang; perubahan bisa hilang tanpa peringatan (kini ada pertanyaan sebelum keluar); pemilih tanggal bisa crash untuk tanggal di luar rentang; tombol usulan meluap di ponsel; Pengaturan Project terbuka di dalam bingkai tab; notifikasi tidak membuka project yang dimaksud; pertanyaan ke Asisten dari layar lain bisa hilang.
  - Asisten tanpa AI kini menampilkan bagian dokumen yang paling mendekati (diberi label jujur) dan memberi saran kata kunci, bukan langsung "tidak ditemukan". Kata pembingkai pertanyaan seperti "berapa banyak" dan "how many" tidak lagi ikut dicocokkan.
  - Test Flutter kini memutar ulang jawaban asli backend untuk project contoh (`backend/scripts/record_app_fixtures.py`): semua layar dicek dalam dua bahasa, ukuran ponsel dan desktop, tema terang dan gelap, plus alur wizard dan interaksi penting. 67 test backend dan 37 test Flutter lolos; 341 kunci teks lengkap di `id` dan `en`.
  - Lebar baris kode Dart dicatat 140 kolom di `app/analysis_options.yaml`, sama dengan penggaris VS Code.
- **2026-09-25**: `DESIGN.md` dibuat lewat `/impeccable document`, dengan North Star *The Steady Supervisor*: teal pembimbing, tenang dan taktil, nada tenang, tepat, dan menyemangati. Yang dihindari: gamifikasi, dashboard korporat, chatbot AI yang mencolok, dan nada menghakimi.
- **2026-09-25**: `PRODUCT.md` dibuat di akar repo lewat `/impeccable init`. Isinya pengguna, tujuan, positioning, batasan, komitmen merek, bukti yang ada, prinsip, dan aksesibilitas; dipakai sebagai acuan kerja desain berikutnya. Dua keputusan baru: pengguna English di luar Indonesia ([[K-006 Pengguna English di luar Indonesia]]) dan platform adaptive ([[K-007 Platform adaptive]]).
