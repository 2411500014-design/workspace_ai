...
- `DESIGN.md` (repo root) is the design system: tokens in its YAML frontmatter, rules in its eight sections (North Star "The Steady Supervisor"); `.impeccable/design.json` carries tonal ramps, motion, shadows and component snippets. Code tokens live in `app/lib/core/theme/`; keep the two in step.
...
- Flutter screen tests (`app/test/screens_test.dart`) replay real API answers for the sample project from `app/test/fixtures/`; after changing a response shape, re-record with `uv run python scripts/record_app_fixtures.py` in `backend/`. Dart is formatted at 140 columns (`app/analysis_options.yaml`); on this laptop bare `dart` is an older WinGet SDK, so format with Flutter's `dart` or through VS Code.
