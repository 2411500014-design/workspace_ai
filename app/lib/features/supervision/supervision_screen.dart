...
class _SupervisionScreenState extends ConsumerState<SupervisionScreen> {
  ...
  Future<void> _save(String projectId) async {
    ...
    setState(() => _busy = true);
    try {
      final suggestion = await ref.read(repositoryProvider).addSupervisionNote(projectId, _notes.text.trim(), _date);
      ref.invalidate(supervisionNotesProvider(projectId));
